...
export default function AppShell({ children }: { children: React.ReactNode }) {
  ...
  const city = (school?.city || '').replace(/\b\w/g, (c: string) => c.toUpperCase());
  ...
  useEffect(() => {
    ...
    if (isDemo && !localStorage.getItem('manchq_demo_dismissed')) {
      setShowSplash(true);
    }
    if (user?.school_id && user.role === 'school_admin' && !isDemo) {
      if (!localStorage.getItem(`manchq_onboarded_${user.school_id}`)) {
        setShowOnboarding(true);
      }
    }
  }, [user?.email, user?.school_id, user?.role, isDemo]);
  ...
  const drawerWidth = typeof window !== 'undefined' ? Math.min(SIDEBAR_W, window.innerWidth * 0.82) : SIDEBAR_W;
  ...
  return (
    <>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', overflow: 'hidden' }}>
        ...
        <div style={{
          position: 'fixed', top: 56, left: 0, bottom: 0, width: drawerWidth,
