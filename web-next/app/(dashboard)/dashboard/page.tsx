...
import { recitals as recitalApi, students as studentApi, batches as batchApi, events as eventApi, todos as todoApi, schools as schoolsApi, schedules as schedulesApi, scheduleExceptions as exceptionsApi } from "@/lib/api";
...
export default function DashboardPage() {
  ...
  // Computed inline (no useMemo) so it always reflects the latest query data
  const _buildCalendarEvents = () => {
  ...
  const allCalendarEvents = _buildCalendarEvents();
  ...
  console.log('%c[Dashboard]', 'color:cyan;font-weight:bold',
    ...
    '| allEvents=', allCalendarEvents.length,
    '| scheduleList[0]=', scheduleList[0] ?? 'empty'
  );
  ...
  const CREATE_OPTIONS = [
    {
      ...
      onClick: () => { router.push('/schedule'); setCreateMenuOpen(false); },
    },
    {
      ...
      onClick: () => { router.push('/recitals?new=1'); setCreateMenuOpen(false); },
    },
    {
      ...
      onClick: () => { router.push('/students'); setCreateMenuOpen(false); },
    },
  ];
  ...
  const widgetsBlock = (
    ...
    <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : 'repeat(3, 1fr)', gap: 18, width: '100%', boxSizing: 'border-box' }}>
      {/* Upcoming Recitals */}
      <Widget title="Upcoming Recitals" onViewAll={() => router.push('/recitals')}>
      ...
      {/* Upcoming Classes */}
      <Widget title="Upcoming Classes" onViewAll={() => router.push('/schedule')}>
      ...
      {/* To-Dos */}
      <Widget title="To-Do" onViewAll={() => router.push('/todos')}>
    ...
    <div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 32, gap: 20 }}>
        ...
        <div style={{ display: 'flex', gap: 10, flexShrink: 0, position: 'relative' }} ref={createMenuRef}>
          ...
          <Button variant="primary" onClick={() => router.push('/schedule')}>View Schedule →</Button>
          {createMenuOpen && (
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 360px', gap: 36, alignItems: 'start' }}>
        ...
        <div>
          {/* THIS WEEK */}
          <SectionTitle first="THIS" accent="WEEK" onViewAll={() => router.push('/schedule')} />
          ...
          <div style={{ background: C.white, borderRadius: 16, border: `3px solid red`, overflow: 'hidden', marginBottom: 36 }}>
          ...
          <SectionTitle first="ALL" accent="RECITALS" onViewAll={() => router.push('/recitals')} />
          {sortedRecitals.length === 0 ? (
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12 }}>
              {sortedRecitals.slice(0, 6).map((r, i) => (
                <RecitalCard key={r.id} r={r} index={i} onClick={() => router.push('/recitals')} />
              ))}
            </div>
          )}
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 28 }}>
          ...
          {sortedRecitals[0] && (
            <div>
              ...
              <FeaturedRecitalCard r={sortedRecitals[0]} onClick={() => router.push('/recitals')} />
            </div>
          )}
