...
import { todos as todosApi, events as eventsApi, recitals as recitalsApi } from '@/lib/api';
...
export default function TodosPage() {
  ...
  const { data: todosData, isLoading } = useQuery<any>({ queryKey: ['todos', sid], queryFn: () => todosApi.list(sid), enabled: !!sid });
  const { data: eventsData } = useQuery<any>({ queryKey: ['events', sid], queryFn: () => eventsApi.list(sid), enabled: !!sid });
  const { data: recitalsData } = useQuery<any>({ queryKey: ['recitals', sid], queryFn: () => recitalsApi.list(sid), enabled: !!sid });

  const todos: any[] = todosData?.todos || [];
  const eventsList: any[] = eventsData?.events || [];
  const recitalsList: any[] = recitalsData?.recitals || [];

  const createMut = useMutation({
    mutationFn: (data: any) => todosApi.create(sid, data),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['todos', sid] });
      setQuickTitle('');
      setExtraDueDate('');
      setExtraAssignedTo('');
      setExtraAssoc('');
    ...
    onError: (err: any) => toast.error(err?.error || err?.message || 'Failed to create'),
  });

  const toggleMut = useMutation({
    mutationFn: (id: any) => todosApi.toggle(sid, id),
    onMutate: async (id: any) => {
      await qc.cancelQueries({ queryKey: ['todos', sid] });
      const prev = qc.getQueryData(['todos', sid]);
      qc.setQueryData(['todos', sid], (old: any) => old?.todos
        ? { ...old, todos: old.todos.map((t: any) => t.id === id ? { ...t, is_complete: t.is_complete ? 0 : 1 } : t) }
        : old);
      return { prev };
    },
    onError: (_e: any, _id: any, ctx: any) => { qc.setQueryData(['todos', sid], ctx.prev); },
    onSettled: () => qc.invalidateQueries({ queryKey: ['todos', sid] }),
  });

  const deleteMut = useMutation({
    mutationFn: (id: any) => todosApi.remove(sid, id),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['todos', sid] }); toast.success('Deleted'); },
    onError: () => toast.error('Failed to delete'),
  });

  const handleAdd = () => {
    if (!quickTitle.trim()) return;
    const payload: any = { title: quickTitle.trim() };
  ...
  const filtered = useMemo(() => {
    ...
    if (activeTab === 'open') return todos.filter((t: any) => !t.is_complete);
    if (activeTab === 'completed') return todos.filter((t: any) => t.is_complete);
    const [type, id] = activeTab.split(':');
    const numId = Number(id);
    if (type === 'event') return todos.filter((t: any) => t.event_id === numId);
    if (type === 'recital') return todos.filter((t: any) => t.recital_id === numId);
    return todos;
  }, [todos, activeTab]);

  const openTodos = filtered.filter((t: any) => !t.is_complete);
  const completedTodos = filtered.filter((t: any) => t.is_complete);
  ...
  return (
    <div>
                ...
                >
                  ...
                  {eventsList.length > 0 && (
                    <optgroup label="Events">
                      {eventsList.map((ev: any) => <option key={`event:${ev.id}`} value={`event:${ev.id}`}>{ev.title}</option>)}
                    </optgroup>
                  )}
                  {recitalsList.length > 0 && (
                    <optgroup label="Recitals">
                      {recitalsList.map((r: any) => <option key={`recital:${r.id}`} value={`recital:${r.id}`}>{r.title}</option>)}
                    </optgroup>
                  )}
                </select>
              </label>
      ...
      <div style={{ display: 'flex', gap: 6, marginBottom: 20, flexWrap: 'wrap' }}>
        {[{ key: 'all', label: 'All' }, { key: 'open', label: 'Open' }, { key: 'completed', label: 'Completed' }, ...linkedTabs].map((tab: any) => (
