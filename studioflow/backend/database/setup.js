/**
 * StudioFlow — Database Setup + Seed
...
async function setup() {
  ...
  console.log('🔧 Setting up StudioFlow database...\n');

  // Create DB
  await conn.query(`CREATE DATABASE IF NOT EXISTS \`${process.env.DB_NAME || 'studioflow'}\` CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci`);
  await conn.query(`USE \`${process.env.DB_NAME || 'studioflow'}\``);
  ...
  await conn.query(`ALTER TABLE batches   AUTO_INCREMENT = 1`);
  ...
  // ── Seed Students ──────────────────────────────────────────
  await conn.query(`
    INSERT INTO students (id, school_id, name, age, phone, guardian_name, guardian_phone, guardian_email, join_date, notes) VALUES
  ...
  // ── Schedules ──────────────────────────────────────────────
  await conn.query(`
    INSERT INTO schedules (id, school_id, batch_id, day_of_week, start_time, end_time, room, frequency) VALUES
  ...
  // ── Recitals ───────────────────────────────────────────────
  await conn.query(`
    INSERT INTO recitals (id, school_id, title, event_date, venue, status, description) VALUES
  ...
  `);
  await conn.query(`
    INSERT INTO recital_tasks (recital_id, task_text, is_done, sort_order) VALUES
    (1,'Book Lincoln Center venue',              1, 1),
    (1,'Confirm costume fittings for all students',1,2),
    (1,'Send parent invitation letters',         1, 3),
    (1,'Book hair & makeup artist',              0, 4),
    (1,'Arrange stage lighting with venue',      0, 5),
    (1,'Print programs & tickets',               0, 6),
    (1,'Final dress rehearsal',                  0, 7),
    (2,'Confirm date with all families',         0, 1),
    (2,'Select repertoire pieces',               0, 2),
    (3,'Register team for competition',          1, 1),
    (3,'Choreograph competition set',            1, 2),
    (3,'Book team travel & accommodation',       0, 3),
    (3,'Order matching team outfits',            0, 4),
    (3,'Submit music tracks to organizers',      0, 5),
    (3,'Run mock battle sessions x3',            0, 6),
    (4,'Set outdoor stage layout',               0, 1),
    (4,'Reach out to local food vendors',        0, 2),
    (4,'Design event flyer',                     0, 3)
  `);
  console.log('✅ Recitals seeded');

  // ── Fee Plans ──────────────────────────────────────────────
  await conn.query(`
    INSERT INTO fee_plans (id, school_id, name, amount, currency, period) VALUES
  ...
  await conn.query(`
    INSERT INTO student_fees (school_id, student_id, fee_plan_id, description, amount, currency, due_date, paid_date, status) VALUES
  ...
  console.log('  School Admin → swapna@rhythmgrace.com   / School123!');
  console.log('  Teacher      → teacher@rhythmgrace.com / Teacher123!');
  console.log('  Parent       → parent@rhythmgrace.com  / Parent123!\n');
}
