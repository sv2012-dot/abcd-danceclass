...
exports.getRecital = async (req, res) => {
  try {
    ...
    // Pull the dedicated public_contact_* fields (set via Team page
    // → Public Contact block). NOT owner_name/email/phone — those are
    // internal-only. The frontend renders only the rows whose value
    // is non-null, so nothing leaks until the admin fills them in.
    const [schools] = await pool.query(
      'SELECT id, name, city, slug, public_contact_name, public_contact_email, public_contact_phone FROM schools WHERE slug = ? AND deleted_at IS NULL LIMIT 1',
      [schoolSlug]
    );
    ...
    if (!recitals[0]) {
      return res.json({
        school: { name: school.name, city: school.city, slug: school.slug, contact_name: school.public_contact_name || null, email: school.public_contact_email || null, phone: school.public_contact_phone || null },
        recital: null,
        cancelled: true,
      });
    }
